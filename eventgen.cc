...
// Bring azimuthal-angle difference within [-pi, pi].
...
// Calculate \DeltaR in eta-phi space.
...
int main() {
  ...
  // --------------------------------------------------------------------------
  // PYTHIA setup: ep collisions at sqrt(s) = 100 GeV.
  // --------------------------------------------------------------------------
  Pythia pythia;
  ...
  pythia.readString("Beams:eA = 10.");  // 10 GeV electron beam
  pythia.readString("Beams:eB = 275."); // 275 GeV proton beam

  // Enable relevant hard processes.
  // Note: this analysis explicitly selects subprocess code 284: gamma + q -> q + g
  ...
  // --------------------------------------------------------------------------
  // Bringing in the output, merged root files from the job, in the current directory
  TFile *fout = new TFile("whoa.root", "RECREATE");

  // Generate 1000000 events
  const int nEvents = 1e6;

  // SlowJet configuration.
  // power = -1 uses anti-kT algorithm for jet clustering.
  // R = 0.5 is the jet radius.
  // pTmin = 5.0 GeV is the minimum reconstructed jet pT.
  const double R = 0.5;
  ...
  // Event iterator.
  // --------------------------------------------------------------------------
  // NOTE: Gluons have PDG code |id| <= 6, gluons are |id| = 21
  for (int event = 0; event < nEvents; event++) {
    ...
    // ------------------------------------------------------------------------
    // Cluster final-state particles into jets using SlowJet.
    // ------------------------------------------------------------------------
    ...
    // the reconstructed jets as unmatched and move to next event.
    if (hardProducts.empty()) {
      numUnmatchedJets += nJets;
      continue;
    }

    // Loop over reconstructed jets and match each to the nearest outgoing
    ...
    for (int j = 0; j < nJets; j++) {
      ...
      // ----------------------------------------------------------------------
      // Fill quark or gluon histograms depending on matched hard parton.
      // ----------------------------------------------------------------------
      if (originPDG == 21) {
        numGluonJets++;
  ...
  // --------------------------------------------------------------------------
  // Compute averages for debugging/observation.
  // --------------------------------------------------------------------------
  ...
  // --------------------------------------------------------------------------
  // Write histograms to ROOT file, to up0load for plotting purposes
  // --------------------------------------------------------------------------
  fout->cd();
  h_quarkMult->Write();
  h_gluonMult->Write();
